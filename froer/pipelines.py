...
class FroerPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('froer.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `froer`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM froer WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO froer (date, title, link, content)"
